...
def parse_protobuf_raw(data):
    """Manually parse protobuf bytes to extract fields"""
    result = {}
    pos = 0
    data_len = len(data)
...
def get_friends_list(token, server_name=None):
    """Get user's friends list using the provided JWT token"""
    try:
        if not server_name:
            server_name = get_server_from_token(token)
        
        # Fixed encrypted payload for GetFriend request
        payload_hex = '362b36f221ac9ed98b104f7b53c858dc'
        payload = bytes.fromhex(payload_hex)
        
        endpoint = get_base_url(server_name) + "GetFriend"
        headers = {
            "Authorization": f"Bearer {token}",
            "User-Agent": USERAGENT,
            "Connection": "Keep-Alive",
        ...
        # Extract friends from field 1 (repeated message)
        friends = []
        if 1 in parsed and isinstance(parsed[1], list):
            for friend in parsed[1]:
                if isinstance(friend, dict):
                    # Field 1 is UID, field 3 is nickname
                    uid = friend.get(1, "Unknown")
        ...
        # Also extract user info from token
...
def get_friends_endpoint():
    """Get friends list endpoint (used by frontend)"""
    token = request.args.get('token')
...
def get_friends_legacy():
    """Legacy get friends list endpoint"""
    token = request.args.get('token')
...
def health_check():
    return jsonify({
        ...
        "endpoints": {
            ...
            "GET /get_friends_list": "Get friends list (legacy)",
